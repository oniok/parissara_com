			...
			{if ($product->show_price && !isset($restricted_country_mode)) || isset($groups) || $product->reference || (isset($HOOK_PRODUCT_ACTIONS) && $HOOK_PRODUCT_ACTIONS)}
				...
				<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
					...
					<div class="box-info-product">
						...
						<div class="group-btn">
							<div class="compare">
								<a class="add_to_compare buttons_bottom_block" href="{$link->getPageLink('products-comparison')|escape:'html':'UTF-8'}" data-id-product="{$product->id|intval}" title="{l s='Add to compare'}" >
									<i class="material-icons">playlist_add</i> <span>{l s='Add to compare'}</span>
								</a>
							</div>
