        ...
        $form_general = array(
            ...
            'input' => array(
                ...
                array(
                    ...
                    'desc' => 'Number of time create random product when showing product carousel has order by Random',
...
//            $order_by_prefix = 'cp';
        ...
        if ($value_by_product_type && $product_type == 'home_featured') {
            ...
            $result = $category->getProducts((int)Context::getContext()->language->id, 1, $n, 'position');
        ...
        if ($value_by_product_type && $product_type == 'best_sellers') {
            ...
            $sql .= ' LEFT JOIN `ps_product` p ON ps.`id_product` = p.`id_product`';
...
//            $sql .= ' ORDER BY product_shop.date_add '.(!$get_total ? ' LIMIT '.(int)$n : '');
    ...
    {
        $this->context->controller->addJS($this->path_resource.'js/ApImage360.js');
        $this->context->controller->addCSS($this->path_resource.'css/ApImage360.css');
    ...
    protected function getCacheId($hook_name = null)
    {
        ...
        if ($this->profile_param && isset($this->profile_param[$hook_name]) && $this->profile_param[$hook_name]) {
            ...
            if (isset($this->profile_param[$hook_name][$current_page])) {
                ...
                $random = round(rand(1, max((int)Configuration::get('APPAGEBUILDER_PRODUCT_MAX_RANDOM'), 1)));
